...
namespace Lucene.Net.Analysis.Sinks
{
    ...
    {
        private readonly ICollection<WeakReference> sinks = new LinkedList<WeakReference>();
        ...
        public override bool IncrementToken()
        {
            if (m_input.IncrementToken())
            {
                ...
                foreach (WeakReference @ref in sinks)
                {
                    SinkTokenStream sink = (SinkTokenStream)@ref.Target;
                    if (@ref.IsAlive)
                    {
                        if (sink.Accept(this))
                        {
